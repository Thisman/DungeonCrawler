...
namespace DungeonCrawler.Gameplay.Battle
{
    ...
    public sealed class BattleGridDragAndDropController : MonoBehaviour
    {
        ...
        private bool CanDrag(Transform draggable)
        {
            ...
            var squadController = draggable.GetComponentInParent<SquadController>();
            var squadModel = squadController?.Model;
        ...
        private bool TryGetPlacementInfo(Transform slot, out Transform resolvedSlot, out Transform occupantToSwap)
        {
            ...
            var draggedController = _draggedObject.GetComponent<SquadController>();
            var draggedDefinition = draggedController?.Model?.Unit?.Definition;
            if (draggedDefinition == null)
            {
                return false;
            }

            var draggedIsEnemy = draggedDefinition.IsEnemy();
            if (side == BattleGridSlotSide.Enemy && !draggedIsEnemy)
            {
                return false;
            }

            if (side == BattleGridSlotSide.Ally && draggedIsEnemy)
            {
                return false;
            }
            ...
            var occupantController = occupant.GetComponent<SquadController>();
            var occupantDefinition = occupantController?.Model?.Unit?.Definition;
            if (occupantDefinition == null)
            {
                return false;
            }

            if (occupantDefinition.IsEnemy() != draggedIsEnemy)
            {
                return false;
            }
